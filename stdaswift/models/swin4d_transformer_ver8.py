...
class WindowAttention4D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        qkv_ = self.qkv(x).reshape(b_, n, 3, self.num_heads, c // self.num_heads)
        qkv = rearrange(qkv_, "b n qkv heads c -> qkv b heads n c", qkv=3)
...
class SwinTransformerBlock4D(nn.Module):
    ...
    def __init__(
        ...
        self.time_attn = TimeAttention(dim, num_heads, qkv_bias, attn_drop, drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()

        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act=act_layer, dropout_rate=drop, dropout_mode="swin")
        self.mlp2 = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act=act_layer, dropout_rate=drop, dropout_mode="swin")
    ...
    def forward_part2(self, x):
        ...
        # original forward part 2
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        
        # Apply Time Attention after Spatial Attention
        x = rearrange(x, "b d h w t c -> (b d h w) t c")
        # x = self.time_attn(x)
        x = self.time_attn(self.norm2(x))
        x = x.view(b, d, h, w, t, c).permute(0, 1, 2, 3, 4, 5)  # Reshape back to (B, D, H, W, T, C)
...
class PatchMergingV2(nn.Module):
    ...
    def forward(self, x):
        ...
        b, d, h, w, t, c = x_shape
        x = torch.cat(
            [x[:, i::2, j::2, k::2, :, :] for i, j in itertools.product(range(2), range(2)) for k in range(2)],
            -1,
        )
...
class PositionalEmbedding(nn.Module):
    ...
    def __init__(
        ...
        Args:
            dim: number of feature channels.
            patch_dim: total number of patches per spatial dimension and time
        """
...
class SwinTransformer4D(nn.Module):
    ...
    def __init__(
        self,
        img_size: Tuple[int, int, int, int] = (96, 96, 96, 15),
        in_chans: int = 1,
        embed_dim: int = 36,
        window_size: Sequence[int] = (6, 6, 6, 1),
        first_window_size: Sequence[int] = (6, 6, 6, 1),
        patch_size: Sequence[int] = (6, 6, 6, 1),
    ...
    def interpolate_time_embed(self, new_time_length):
        """
        Interpolate the time embedding to a new length.
        
        Args:
            new_time_length (int): The new length of the time dimension.
        """
        for i, pos_embed in enumerate(self.pos_embeds):
            # Get the current time embedding
            current_time_embed = pos_embed.time_embed  # Shape: [1, dim, 1, 1, 1, t]
            
            # Interpolate to the new time length
            new_time_embed = F.interpolate(
                rearrange(current_time_embed, '1 dim 1 1 1 t -> 1 dim t'),  # Shape: [1, dim, t]
                size=(new_time_length,),
                mode='linear',
                align_corners=False
            )
            
            # Reshape back to the original shape
            new_time_embed = rearrange(new_time_embed, '1 dim t -> 1 dim 1 1 1 t')  # Shape: [1, dim, 1, 1, 1, new_t]
            
            # Update the time embedding
            pos_embed.time_embed = nn.Parameter(new_time_embed)
            
        print(f"Time embeddings interpolated to new length: {new_time_length}")
